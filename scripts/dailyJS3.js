if (location.pathname.indexOf("leader_guild.php") >= 0) {
	...
	for (let i = 0; i < tr.length; i++) {
		...
		if ((checkSum > 0) && (hp > 0)) {
			// tr[i].querySelector("td").innerHTML += "<div style = 'margin-left:10px'><a href = 'https://daily.heroeswm.ru/leader/bandits.php?hp=" + hp + "&sum=" + checkSum + "'>   Daily</a></div>";
		}
	}
}
    ...
    if ((typeof(stage) !== 'undefined') && (typeof(stage.pole) !== 'undefined') && (typeof(stage.pole.onMouseMoveFlash) === "function")) {
		...
		stage.pole.showmagicbook = function (page, iszak, check_spell) {
			...
			function proccedinbook(so, powered, showed_cnt) {
				...
				if (magicbooknames[magicbookspells.indexOf(s)].includes("")) {
					s_name += "";
				}				
		...
		div.innerHTML = "<div class='info_row' id='atb-start-bonus' onmouseup = 'checkSet(\"atbStartDisplay\")'><label class='checkbox_container'>  <input type='checkbox'" + (checkTrue("atbStartDisplay") ? " checked ":"") + "id='atbStartDisplay_checkbox'><span class='checkbox_checkmark'></span></label></div><br>"
        ...
        effectsDisplay.setAttribute("title", "          ");
        ...
        window.testBlock = function(i = 0, firewall = false) {		
			...
			if ((firewall == true) && (i != lastFire) && (i > 0)) {
				...
				div.innerHTML = "<center> " + i + " .</center>";	
				lastFire = i;
				fireInfo = true;
			} else if (firewall == false) {
				fireInfo = false;
				if ((i == 0) || (stage.pole.showmagicinfo(i, 1).indexOf(" ") != -1)) {
					div.innerHTML = "";
					div.style.height = "0";
					lastFire = 0;
					return 0;
        ...
        let spanHTML = (damageTableDisplay == "table" ? "()" : "()");
        ...
        window.setDamageTableDisplay = function() {
            ...
            spanHTML = (damageTableDisplay == "table" ? "()" : "()");
            updateDamageData();
        }
        window.updateDamageData = function() {
            ...
            let txt = "<B> </B> <span onclick = 'setDamageTableDisplay()' title '/' style = 'font-size:80%;cursor:pointer'>" + spanHTML + "</span><table width = 100% style = 'display:" + damageTableDisplay + "'><tbody><tr>";
            for (let i = 0; i < 2; i++) {
        ...
        stage.pole.procceddamage = window.procceddamage = function(i) {
            ...
            if ((this.obj[i].damage != 0) || (!this.obj[i].runaway)) {
                ...
                if (this.obj[i].incorporeal) {
                    if ((this.obj[i].magicdamage) && (this.obj[i].magicdamage != " ")) {
        ...
        stage.pole.process_luck = window.process_luck = function(current) {
            ...
            if (lucky == 'badmorale') {
                switch (lang) {
                    case 0:
                        htmllog += '' + this.html('name', current) + '   ' + this.html('end');
                        break;
                    case 1:
            ...
            if ((firstbattle) && (!buttons_visible['win_dialog'])) {
                if ((firstbattle == 2) && (btype == 87)) {
                    if ((!buttons_visible['win_Mission']) && (!finished) && (showed_hint[2] == 0) && (activeobj > 0) && (inserted)) {
                        ...
                        if (lang == 0) {
                            document.getElementById('mission_header').innerHTML = 'Задание: захватить шахту';
                            document.getElementById('win_Mission_txt').innerHTML = 'Теперь в вашем распоряжении целая армия, состоящая из различных отрядов. Текущая задача - разгромить пограничников и захватить шахту. \n\nВы можете сами наносить удары на расстоянии, а так же давать любые указания своим отрядам.\nВаш союзник-эльфийка поможет победить!';
                        }
                        if (lang == 1) {
                ...
                if (firstbattle == 2) {
                    ...
                    if ((!buttons_visible['win_Mission']) && (showed_hint[2] == 0) && (activeobj > 0) && (inserted)) {
                        if ((demomode) && (lang == 0)) {
                            document.getElementById('mission_header').innerHTML = 'Пробный бой';
                            texts[2] = 'Вы находитесь в новой игре Герои Войны и Денег. Это тактическая стратегия с элементами RPG и экономики. Здесь вы сможете реализовать все свои способности, постоянно развиваясь и улучшая свои навыки. Регулярные обновления сохранят ваш интерес к игре надолго.\n\nПобеди в бою и начинай играть!';
                        }
                        if ((demomode) && (lang == 1)) {
        ...
        stage.pole.showmagicinfo = window.showmagicinfo = function(i, t = 0) {
            ...
            if (!enter) {
                if (lang == 0) {
                    dat += '<tr><td></td><td width=10></td><td></td><td width=10></td><td></td></tr>';
                }
                if (lang == 1) {
...
//						  textBaseline: 'middle',
			...
			if (this.obj[i].hero){
				switch (lang){
					case 0:
							ntxt = this.obj[i].nametxt+' ['+this.obj[i].maxhealth+' .]';
					break;
					case 1:
				...
				if (this.obj[i].boss>0){
					switch (lang){
						case 0:
								ntxt = this.obj[i].nametxt+' ['+this.obj[i].boss+' .]';
						break;
						case 1:
